...
namespace nanoFramework.Tools.UnitTester
{
	...
	internal interface IFirmwareFlasher
	{
		...
		void FlashLatestFirmware(string comPort, string testLibrary);
		...
		void FlashFirmwareImage(string comPort, string firmwareTag, string testLibrary);
	}
	...
	internal class Esp32FirmwareFlasher : IFirmwareFlasher
	{
		...
		/// </summary>
		private readonly string _espFirmwareFlasherPath;
		...
		internal Esp32FirmwareFlasher(string espFirmwareFlasherDirectory)
		{
			...
			if (!File.Exists(_espFirmwareFlasherPath)) throw new FileNotFoundException(_espFirmwareFlasherPath);
		}
		...
		public void FlashLatestFirmware(string comPort, string testLibrary)
		{
			// using the EspFirmwareFlasher tool
			var firmwareFlasher = Process.Start(_espFirmwareFlasherPath, $"--port={comPort} --application={testLibrary}");
			firmwareFlasher.WaitForExit();
			if (firmwareFlasher.ExitCode != 0) throw new ApplicationException($"The Esp32FirmwareFlasher process has exited with exit code {firmwareFlasher.ExitCode}");
		}
		...
		/// <param name="comPort">The COM port on which a device is connected</param>
		/// <param name="firmwareTag">The firmware tag which should be flahsed; for ESP32 e.g. 0.1.0-preview.738</param>
		public void FlashFirmwareImage(string comPort, string firmwareTag, string testLibrary)
		{
			// using the EspFirmwareFlasher tool
			var firmwareFlasher = Process.Start(_espFirmwareFlasherPath, $"--port={comPort} --firmware_tag={firmwareTag} --application={testLibrary}");
			firmwareFlasher.WaitForExit();
			if (firmwareFlasher.ExitCode != 0) throw new ApplicationException($"The Esp32FirmwareFlasher process has exited with exit code {firmwareFlasher.ExitCode}");
		}
